...
// Conditionally import expo-notifications - unavailable in Expo Go SDK 53+.
...
try {
  ...
  console.log(
    "[Admin Notifications] expo-notifications not available (expected in Expo Go SDK 53+)",
  );
}
...
function isNotificationsAvailable() {
  ...
  if (isExpoGo) {
    console.log(
      "[Admin Notifications] Running in Expo Go - notifications disabled",
    );
    return false;
  }
...
export async function registerForPushNotifications(userId) {
  if (!isNotificationsAvailable()) {
    console.log(
      "[Admin Notifications] Notifications unavailable - skipping registration",
    );
    return null;
  }
...
export function setupNotificationListeners(
  ...
  if (!isNotificationsAvailable()) {
    console.log(
      "[Admin Notifications] Notifications unavailable - listeners disabled",
    );
    return { remove: () => {} };
  }
...
export async function sendLocalNotification(title, body, data = {}) {
  if (!isNotificationsAvailable()) {
    console.log(
      "[Admin Notifications] Notifications unavailable - local notification skipped",
    );
    return;
  }
