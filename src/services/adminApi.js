import { supabase, createStatelessSupabaseClient } from "../config/supabase";

function extractStoragePathFromPublicUrl(url) {
  if (!url || typeof url !== "string") return null;

  const marker = "/storage/v1/object/public/chawp/";
...
async function deleteStorageImagesByUrls(urls = []) {
  const uniquePaths = [
    ...new Set(
      (urls || [])
  ...
  const { error } = await supabase.storage.from("chawp").remove(uniquePaths);
  if (error) {
    console.warn("Failed to delete one or more storage images:", error.message);
  }
}
...
export async function createVendor(vendorData) {
  try {
    ...
    // Step 1: Create auth user with isolated client so admin session is not replaced
    ...
    // Step 2: Create user profile using the new user's token to satisfy RLS auth.uid() checks.
    const newUserAccessToken = authData?.session?.access_token || null;

    if (!newUserAccessToken) {
      throw new Error(
        "Vendor account created but no session token returned for profile setup. Ensure email confirmation is disabled for admin-created users or use an admin edge function.",
      );
    }

    const profileClient = createStatelessSupabaseClient(newUserAccessToken);
...
export async function updateMeal(mealId, mealData, options = {}) {
  try {
    const { data, error } = await supabase
      .from("chawp_meals")
      .update(mealData)
    ...
    if (options.removedImageUrls?.length) {
      await deleteStorageImagesByUrls(options.removedImageUrls);
    }
...
export async function createDeliveryPersonnel(deliveryData) {
  try {
    ...
    // Step 1: create auth user with isolated client so admin session is preserved
    const authClient = createStatelessSupabaseClient();
    const { data: authData, error: authError } = await authClient.auth.signUp({
      email,
      password,
    ...
    // Step 2: create user profile with role 'delivery' using new user's token for RLS compliance.
    const newUserAccessToken = authData?.session?.access_token || null;

    if (!newUserAccessToken) {
      throw new Error(
        "Delivery account created but no session token returned for profile setup. Ensure email confirmation is disabled for admin-created users or use an admin edge function.",
      );
    }
...
/**
 * Check Paystack subaccount status for delivery personnel
 * @param {string} deliveryPersonnelId - Delivery personnel ID
 */
export async function checkDeliverySubaccount(deliveryPersonnelId) {
  try {
    ...
    if (error) throw error;
    if (!data?.success) {
      throw new Error(data?.error || "Failed to check delivery subaccount");
    }

    return { success: true, data: data.data || null };
  } catch (error) {
    console.error("Error checking delivery subaccount:", error);
    return { success: false, error: error.message };
  }
}

/**
 * Set Paystack subaccount verification status for delivery personnel
 * @param {string} deliveryPersonnelId - Delivery personnel ID
 * @param {boolean} verified - Verification status
 * @param {string|null} subaccountCode - Optional subaccount code override
 */
export async function setDeliverySubaccountVerification(
  deliveryPersonnelId,
  verified,
  subaccountCode = null,
) {
  try {
    const preflight = await checkDeliverySubaccount(deliveryPersonnelId);
    if (!preflight.success) {
      throw new Error(preflight.error || "Failed to check delivery subaccount");
    }

    if (!preflight.data?.exists || preflight.data?.deleted_from_paystack) {
      throw new Error(
        "Subaccount is missing on Paystack. Ask delivery personnel to save payout details again to recreate it.",
      );
    }

    const keyMode = String(
      preflight.data?.diagnostics?.key_mode || "unknown",
    ).toLowerCase();
    const paystackVerificationStatus =
      preflight.data?.diagnostics?.paystack_verification_status || "unverified";

    if (
      Boolean(verified) &&
      !Boolean(preflight.data?.paystack_account_verified) &&
      keyMode !== "test"
    ) {
      throw new Error(
        `Paystack still shows this subaccount as unverified (status: ${paystackVerificationStatus}, mode: ${keyMode}). Verify it in Paystack Dashboard (Subaccounts > select account > Verify), then refresh and try again.`,
      );
    }
    ...
    const { data, error } = await supabase.functions.invoke(
      ...
      {
        body: {
          action: "set_delivery_account_verification",
          delivery_personnel_id: deliveryPersonnelId,
    ...
    if (!data?.success) {
      throw new Error(
        data?.error || "Failed to update delivery subaccount verification",
      );
    }

    return { success: true, data: data.data || null };
  } catch (error) {
    const message = await parseFunctionsHttpError(
      error,
      "Failed to update delivery subaccount verification",
    );
    console.error("Error setting delivery subaccount verification:", message);
...
export async function fetchUserVendorBankDetails(userId) {
  try {
    ...
    const { data: vendor, error: vendorError } = await supabase
      .from("chawp_vendors")
      .select("id, payment_account, account_verified, payment_platform")
      .eq("user_id", userId)
      .single();
...
/**
 * Check Paystack subaccount status for a vendor
 * @param {string} vendorId - Vendor ID
 */
export async function checkVendorSubaccount(vendorId) {
  try {
    const { data, error } = await supabase.functions.invoke(
      "create_subaccount",
      {
        body: {
          action: "check_subaccount",
          vendor_id: vendorId,
        },
      },
    );
    ...
    if (error) throw error;
    if (!data?.success) {
      throw new Error(data?.error || "Failed to check vendor subaccount");
    }

    return { success: true, data: data.data || null };
  } catch (error) {
    console.error("Error checking vendor subaccount:", error);
    return { success: false, error: error.message };
  }
}

async function parseFunctionsHttpError(error, fallbackMessage) {
  const context = error?.context;

  try {
    if (context?.json && !context?.bodyUsed) {
      const payload = await context.json();
      return (
        payload?.error || payload?.message || error?.message || fallbackMessage
      );
    }
  } catch (_parseError) {
  ...
  try {
    if (context?.text && !context?.bodyUsed) {
      const raw = await context.text();
      if (!raw) return error?.message || fallbackMessage;
      ...
      try {
        ...
        return raw || error?.message || fallbackMessage;
      }
    }
  } catch (_textError) {
    // Ignore parse errors and fallback below.
...
/**
 * Set Paystack subaccount verification status for a vendor
 * @param {string} vendorId - Vendor ID
 * @param {boolean} verified - Verification status
 * @param {string|null} subaccountCode - Optional subaccount code override
 */
export async function setVendorSubaccountVerification(
  vendorId,
  verified,
  subaccountCode = null,
) {
  try {
    // Preflight check so we can return a clean message when the subaccount
    // is missing/deleted, instead of a generic non-2xx edge function error.
    const preflight = await checkVendorSubaccount(vendorId);
    if (!preflight.success) {
      throw new Error(preflight.error || "Failed to check vendor subaccount");
    }

    if (!preflight.data?.exists || preflight.data?.deleted_from_paystack) {
      throw new Error(
        "Subaccount is missing on Paystack. Ask vendor to save payout details again to recreate it.",
      );
    }
    ...
    if (
      ...
      throw new Error(
        `Paystack still shows this subaccount as unverified (status: ${paystackVerificationStatus}, mode: ${keyMode}). Verify it in Paystack Dashboard first (Subaccounts > select account > Verify), then refresh and try again.`,
      );
    }
    ...
    const { data, error } = await supabase.functions.invoke(
      ...
      {
        body: {
          action: "set_account_verification",
          vendor_id: vendorId,
          verified: Boolean(verified),
          ...(subaccountCode ? { subaccount_code: subaccountCode } : {}),
        },
    ...
    if (!data?.success) {
      throw new Error(
        data?.error || "Failed to update vendor subaccount verification",
      );
    }
    ...
    const message = await parseFunctionsHttpError(
      error,
      "Failed to update vendor subaccount verification",
    );
    console.error("Error setting vendor subaccount verification:", message);
    return { success: false, error: message };
  }
}
