...
export default function SettingsPage() {
  ...
  const handleSave = async () => {
    try {
      ...
      const serviceFeePercentageNum = parseFloat(serviceFeePercentage);

      if (
        serviceFeeMode === "flat" &&
        (isNaN(serviceFeeNum) || serviceFeeNum < 0)
      ) {
      ...
      if (
        ...
        (isNaN(serviceFeePercentageNum) || serviceFeePercentageNum < 0)
      ) {
        Alert.alert(
          "Invalid Input",
          "Service fee percentage must be a valid number",
        );
        return;
      }
          ...
          <View style={styles.feeModeRow}>
            <TouchableOpacity
              style={[
                ...
                serviceFeeMode === "flat" && styles.feeModeButtonActive,
              ]}
              onPress={() => setServiceFeeMode("flat")}
              disabled={saving}
            >
              <Text
                style={[
                  styles.feeModeButtonText,
                  serviceFeeMode === "flat" && styles.feeModeButtonTextActive,
                ]}
              >
                Flat
              </Text>
            ...
            <TouchableOpacity
              style={[
                styles.feeModeButton,
                serviceFeeMode === "percentage" && styles.feeModeButtonActive,
              ]}
              onPress={() => setServiceFeeMode("percentage")}
              disabled={saving}
            >
              <Text
                style={[
                  styles.feeModeButtonText,
                  serviceFeeMode === "percentage" &&
                    styles.feeModeButtonTextActive,
                ]}
              >
                Percentage
              </Text>
        ...
        <View style={styles.previewCard}>
          <View style={styles.previewRow}>
            ...
            <Text style={styles.previewValue}>
              {serviceFeeMode === "percentage"
                ? `${parseFloat(serviceFeePercentage || 0).toFixed(2)}%`
          ...
          <View style={styles.previewRow}>
            ...
            <Text style={styles.previewTotal}>
              {serviceFeeMode === "percentage"
                ? `${parseFloat(serviceFeePercentage || 0).toFixed(2)}% + GH₵ ${parseFloat(deliveryFee || 0).toFixed(2)}`
                : `GH₵ ${(parseFloat(serviceFee || 0) + parseFloat(deliveryFee || 0)).toFixed(2)}`}
            </Text>
          </View>
        </View>
      </View>
