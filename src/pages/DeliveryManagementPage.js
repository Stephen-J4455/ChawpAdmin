...
import {
  ...
  checkDeliverySubaccount,
  setDeliverySubaccountVerification,
...
export default function DeliveryManagementPage() {
  ...
  const handleViewPerson = async (person) => {
    ...
    const subaccountResult = await checkDeliverySubaccount(person.id);
    if (subaccountResult.success) {
      setSubaccountStatus(subaccountResult.data);
    } else {
      showError(
        "Error",
        subaccountResult.error || "Failed to load delivery subaccount status",
      );
    }
  ...
  const handleToggleBankVerification = async () => {
    ...
    if (!selectedPerson?.id) {
      showError("Error", "No delivery personnel selected");
      return;
    }

    const nextVerifiedState = !bankDetails.is_verified;

    const latestStatusResult = await checkDeliverySubaccount(selectedPerson.id);
    if (!latestStatusResult.success) {
      showError(
        "Error",
        latestStatusResult.error || "Failed to check delivery subaccount",
      );
      return;
    }
    ...
    if (!latestStatus?.subaccount_code || !latestStatus?.exists) {
      showError("Error", "No Paystack subaccount found for this personnel");
      return;
    }
    ...
    if (latestStatus?.deleted_from_paystack) {
      showError(
        "Subaccount Missing",
        "This subaccount no longer exists on Paystack. Ask the delivery personnel to save payout details again to recreate it.",
      );
      return;
    }
    ...
    if (nextVerifiedState && !latestStatus?.paystack_account_verified) {
      showError(
        ...
        "Verify this subaccount in Paystack Dashboard first, then use this button to approve payment details.",
      );
      return;
    }
    ...
    try {
      const bankResult = await verifyDeliveryBankDetails(
        bankDetails.id,
        nextVerifiedState,
      );
      if (!bankResult.success) {
        throw new Error(
          bankResult.error || "Failed to update payment details verification",
        );
      }

      const subaccountResult = await setDeliverySubaccountVerification(
        selectedPerson.id,
        nextVerifiedState,
      ...
      if (!subaccountResult.success) {
        throw new Error(
          subaccountResult.error ||
            "Failed to update payout account verification",
        );
      }

      const refreshedStatusResult = await checkDeliverySubaccount(
        selectedPerson.id,
      );
      if (refreshedStatusResult.success) {
        setSubaccountStatus(refreshedStatusResult.data);
      }
      ...
      showSuccess(
        "Success",
        nextVerifiedState
          ? "Payment details and payout account verified"
          : "Payment details and payout account unverified",
      );
      await loadPersonnel();
    } catch (error) {
  ...
  const isPaymentVerifyDisabled =
    verifyingBank ||
    loadingSubaccount ||
    !bankDetails?.id ||
    !subaccountStatus?.subaccount_code ||
    !subaccountStatus?.exists ||
    subaccountStatus?.deleted_from_paystack ||
    (!bankDetails?.is_verified && !subaccountStatus?.paystack_account_verified);
                    ...
                    <View style={styles.bankDetailsCard}>
                      ...
                      >
                        ...
                        {loadingSubaccount ? (
                          ...
                          <>
                            <View style={styles.bankDetailRow}>
                              <Text style={styles.bankDetailLabel}>Code</Text>
                              <Text style={styles.bankDetailValue}>
                                {subaccountStatus.subaccount_code || "N/A"}
                              </Text>
                            </View>
                            <View style={styles.bankDetailRow}>
                              <Text style={styles.bankDetailLabel}>Exists</Text>
                              <Text style={styles.bankDetailValue}>
                                {subaccountStatus.exists
                                  ? "Exists on Paystack"
                                  : "Not found on Paystack"}
                              </Text>
                            </View>
                            <View style={styles.bankDetailRow}>
                              <Text style={styles.bankDetailLabel}>
                            ...
                            {subaccountStatus.deleted_from_paystack ? (
                              <Text style={styles.payoutWarning}>
                                ⚠ Subaccount is missing on Paystack. Ask
                                delivery personnel to save payment details again
                                to recreate it.
                              </Text>
                            ) : null}
                            ...
                            {!subaccountStatus.paystack_account_verified ? (
                              <Text style={styles.payoutWarning}>
                                ⚠ Paystack subaccount is unverified. Verify it
                                in Paystack Dashboard before enabling payment
                                verification.
                              </Text>
                            ) : null}
                          </>
