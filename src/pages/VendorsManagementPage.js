...
import {
  ...
  setVendorSubaccountVerification,
  fetchVendorPayouts,
  updatePayoutStatus,
...
export default function VendorsManagementPage() {
  ...
  const uploadVendorImage = async (uri, vendorId) => {
    try {
      setUploadingImage(true);
      if (!vendorId) throw new Error("Vendor ID is required for image upload");

      const fileName = `vendor_${vendorId}_${Date.now()}.jpg`;
      const filePath = `vendors/${vendorId}/avatar/${fileName}`;
  ...
  const handleSave = async () => {
    ...
    if (editingVendor) {
      ...
      if (selectedImageUri) {
        try {
          imageUrl = await uploadVendorImage(
            selectedImageUri,
            editingVendor.id,
          );
        } catch (error) {
      ...
      // Create vendor first to get a stable vendor ID, then upload avatar
      result = await createVendor({
        ...formData,
        image: null,
      });

      if (result.success && selectedImageUri && result.data?.id) {
        try {
          const imageUrl = await uploadVendorImage(
            selectedImageUri,
            result.data.id,
          );
          const updateResult = await updateVendor(result.data.id, {
            image: imageUrl,
          });
          ...
          if (!updateResult.success) {
            showError(
              "Error",
              updateResult.error || "Failed to save vendor image",
            );
            return;
          }
        } catch (error) {
          showError("Error", "Vendor created, but image upload failed.");
          return;
        }
      }
    }
  ...
  const handleViewVendorDetails = async (vendor) => {
    ...
    if (vendor.user_id) {
      ...
      if (bankResult.success) {
        ...
        const subaccountResult = await checkVendorSubaccount(vendor.id);
        if (subaccountResult.success) {
          setSubaccountStatus(subaccountResult.data);
        } else {
          showError(
            "Error",
            subaccountResult.error || "Failed to load subaccount status",
          );
        }
  ...
  const handleVerifySubaccount = async () => {
    if (!selectedVendor?.id) {
      showError("Error", "No Paystack subaccount found for this vendor");
      return;
    }

    if (!bankDetails?.id) {
      showError("Error", "No bank details found for this vendor");
      return;
    }

    const latestStatusResult = await checkVendorSubaccount(selectedVendor.id);
    if (!latestStatusResult.success) {
      showError(
        "Error",
        latestStatusResult.error || "Failed to check vendor subaccount",
      );
      return;
    }

    const latestStatus = latestStatusResult.data;
    setSubaccountStatus(latestStatus);

    if (!latestStatus?.subaccount_code || !latestStatus?.exists) {
      showError("Error", "No Paystack subaccount found for this vendor");
      return;
    }

    if (latestStatus?.deleted_from_paystack) {
      showError(
        "Subaccount Missing",
        "This subaccount no longer exists on Paystack. Ask the vendor to save payout details again to recreate it.",
      );
      return;
    }

    const currentlyVerified =
      Boolean(bankDetails.is_verified) &&
      Boolean(latestStatus.account_verified);
    const nextVerifiedState = !currentlyVerified;

    if (nextVerifiedState && !latestStatus.paystack_account_verified) {
      showError(
        "Paystack Verification Required",
        "Verify this subaccount in Paystack Dashboard first, then use this button to complete verification and payout approval.",
      );
      return;
    }
    ...
    try {
      if (Boolean(bankDetails.is_verified) !== nextVerifiedState) {
        const bankResult = await verifyVendorBankDetails(
          bankDetails.id,
          nextVerifiedState,
        );
        if (!bankResult.success) {
          throw new Error(
            bankResult.error || "Failed to update bank verification",
          );
        }
      ...
      if (Boolean(latestStatus.account_verified) !== nextVerifiedState) {
        const subaccountResult = await setVendorSubaccountVerification(
          selectedVendor.id,
          nextVerifiedState,
          latestStatus.subaccount_code,
        );
        if (!subaccountResult.success) {
          throw new Error(
            subaccountResult.error ||
              "Failed to update subaccount verification",
          );
        }
      }

      const refreshed = await checkVendorSubaccount(selectedVendor.id);
      if (refreshed.success) {
        setSubaccountStatus(refreshed.data);
      }

      showSuccess(
        "Success",
        nextVerifiedState
          ? "Bank details and Paystack setup verified and approved for payout"
          : "Bank details and Paystack approval removed",
      );
    } catch (error) {
      showError("Error", error?.message || "Failed to update verification");
    } finally {
  ...
  const isCombinedVerificationDisabled =
    verifyingBank ||
    verifyingSubaccount ||
    !bankDetails?.id ||
    !subaccountStatus?.subaccount_code ||
    subaccountStatus?.deleted_from_paystack ||
    (!(bankDetails?.is_verified && subaccountStatus?.account_verified) &&
      !subaccountStatus?.paystack_account_verified);
            ...
            <ScrollView style={styles.modalForm}>
              ...
              <View style={styles.detailSection}>
                ...
                {loadingBankDetails ? (
                  ...
                  <View style={styles.bankDetailsCard}>
                    ...
                    <View
                      style={[styles.detailSection, { marginTop: spacing.md }]}
                    >
                      ...
                      {loadingSubaccount ? (
                        ...
                        <>
                          <View style={styles.bankDetailsRow}>
                            <Text style={styles.bankLabel}>Code:</Text>
                            <Text style={styles.bankValue}>
                              {subaccountStatus.subaccount_code || "N/A"}
                            </Text>
                          </View>
                          <View style={styles.bankDetailsRow}>
                            <Text style={styles.bankLabel}>Status:</Text>
                            <Text style={styles.bankValue}>
                              {subaccountStatus.account_verified
                                ? "Verified & Approved for Payout"
                          ...
                          {subaccountStatus.deleted_from_paystack ? (
                            <Text style={styles.payoutWarning}>
                              ⚠ Subaccount missing on Paystack (needs
                              recreation)
                            </Text>
                          ) : null}
                          ...
                          {subaccountStatus.subaccount_code ? (
                            <TouchableOpacity
                              style={[
                                ...
                                isCombinedVerificationDisabled &&
                                  styles.disabledVerifyButton,
                                { marginTop: spacing.sm },
                              ]}
                              onPress={handleVerifySubaccount}
                              disabled={isCombinedVerificationDisabled}
                            >
                              {verifyingSubaccount || verifyingBank ? (
                                ...
                                <Text
                                  style={[
                                    ...
                                    isCombinedVerificationDisabled &&
                                      styles.disabledVerifyButtonText,
                                  ]}
                                >
                                  {bankDetails?.is_verified &&
                                  subaccountStatus.account_verified
                                    ? "Remove Bank + Paystack Approval"
                                    : "Verify Bank + Approve Payout"}
                                </Text>
                              )}
                            </TouchableOpacity>
                          ) : null}
                          ...
                          {!subaccountStatus.paystack_account_verified && (
                            <Text style={styles.payoutWarning}>
                              ⚠ Paystack subaccount is unverified. Verify it in
                              Paystack Dashboard before this combined button can
                              be used.
                            </Text>
                          )}
                        </>
                      ) : (
                        <Text style={styles.emptyText}>No subaccount data</Text>
                      )}
                    </View>
                  </View>
                ) : (
              ...
              <View style={styles.detailSection}>
                ...
                {(!bankDetails?.is_verified ||
                  ...
                  bankDetails && (
                    <Text style={styles.payoutWarning}>
                      ⚠ Verify bank details and apply internal subaccount
                      approval before creating payouts
                    </Text>
                  )}
